...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (req.userId === Number(provider_id)) {
      return res
        .status(401)
        .json({ error: ' Not allow create appointment with yourself' });
    }
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateSubHours, new Date())) {
      return res.status(401).json({
        error: 'Limit to cancel the appointment is 2 hours in advance',
      });
    }
    ...
    await Mail.sendMail({
      ...
      context: {
        ...
        date: format(appointment.date, "MMMM do 'at' H:mm'h'"),
      },
    });
